...
LONG MiniDumper::TopLevelFilter( struct _EXCEPTION_POINTERS *pExceptionInfo )
{
	...
	if (hDll)
	{
		...
		if (pDump)
		{
			...
			if (!GetCurrentDirectory( _MAX_PATH, szDumpPath ))
			...
			::MessageBox(NULL, "Something bad happened in your program, a diagnostic file will be created", m_szAppName, MB_YESNO);
			{
				// create the file
				...
				if (hFile!=INVALID_HANDLE_VALUE)
				{
					...
					if (bOK)
					{
						sprintf( szScratch, "Saved dump file to '%s', please send it to the developer", szDumpPath );
						szResult = szScratch;
