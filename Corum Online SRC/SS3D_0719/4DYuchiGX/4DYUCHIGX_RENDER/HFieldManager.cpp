...
BOOL __stdcall CHFieldManager::LoadTilePalette(TEXTURE_TABLE* pTexTable,DWORD dwTileTextureNum)
{
	...
	{
		if (!m_pRenderer->AllocTexture(m_pTileTexture+i,pTexTable[i].szTextureName,0,TRUE,TRUE))
		{
...
BOOL __stdcall CHFieldManager::ReplaceTile(char* szFileName,DWORD dwTexIndex)
{
	...
	if (!m_pRenderer->AllocTexture(&tex,szFileName,0,TRUE,TRUE))
		goto lb_return;

	m_pTileTexture[dwTexIndex].Release();
	m_pRenderer->AllocTexture(&m_pTileTexture[dwTexIndex],szFileName,0,TRUE,TRUE);

lb_return:
	return bResult;
}
BOOL __stdcall CHFieldManager::CreateIndexBuffer(
	DWORD dwIndicesNum,
	DWORD dwDetailLevel,
...
void __stdcall CHFieldManager::UnlcokIndexBufferPtr(
	DWORD /*dwDetailLevel*/,
	DWORD /*dwPositionMask*/
	)
{
