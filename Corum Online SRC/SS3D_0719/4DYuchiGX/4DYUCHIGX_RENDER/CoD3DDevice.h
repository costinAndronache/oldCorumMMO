...
//	CProjectedIBManager*			GetPrjIBManager()		{return &m_PrjIBManager;}
	...
	BOOL							AllocTexture(CTexture* pTextureHandle,TCHAR* uszFileName,DWORD dwWidthHeight,BOOL bUseMipmap,BOOL bCompress) {return m_pTextureManager->AllocTexture(pTextureHandle,uszFileName,dwWidthHeight,bUseMipmap,bCompress);}
	void							AllocDefaultTexture(CTexture* pTexHandle) {m_pTextureManager->AllocDefaultTexture(pTexHandle);}
	...
	void							SetDefaultTexture(LPDIRECT3DDEVICE8 pDevice,DWORD dwTexStageIndex) {m_pTextureManager->SetDefaultTexture(pDevice,dwTexStageIndex);}
