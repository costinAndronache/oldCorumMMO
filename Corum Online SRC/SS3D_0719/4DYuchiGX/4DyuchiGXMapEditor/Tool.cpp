...
//	m_pExecutive->Initialize(g_pMainFrame->m_hWnd,NULL,4096,512,512);
	...
	m_pPalette = new CDialogTilePalette;
...
void CTool::OnRClickGXObject(POINT* ptCursor,UINT nFlags)
{
	...
	GXOBJECT_HANDLE	gxo = m_pExecutive->GXOGetObjectWithScreenCoord(&v3IntersectPoint,&dwModelIndex,&dwObjIndex,&fDist,ptCursor,m_dwCurrentViewportIndex,0);
...
void CTool::SaveMap(char* szFileName)
{
	...
	if (m_pStaticModel)
	{
		if (m_pStaticModel->GetObjectNum())
		{
			...
			RemoveExt(stm,MAX_NAME_LEN,stm);
			lstrcat(stm,".stm");
	...
	if (m_pHField)
	{
		...
		RemoveExt(hfl,MAX_NAME_LEN,hfl);
		lstrcat(hfl,".hfl");
