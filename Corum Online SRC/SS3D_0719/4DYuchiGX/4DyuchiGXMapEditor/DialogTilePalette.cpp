...
BOOL CDialogTilePalette::Initialize(DWORD dwTileSize,CDialogEditEntry* pEditEntryDlg)
{
	//yh
...
void CDialogTilePalette::OnButtonLoadPalette() 
{
	...
	LPCTSTR filename = pathname.GetString();
...
//	char szFilters[] = "TGA Files(*.tga) | *.tga | TIF Files(*.tif) | *.tif||";
	...
	IDISpriteObject*	pSprite;
	pSprite = m_pRenderer->CreateSpriteObject((char*)filename,SPRITE_CREATE_IMAGE);
	if (!pSprite)
		return;
	...
	RenderPalette();
	g_pTool->ReplaceTile((char*)filename,m_wSelectedTileIndex);
