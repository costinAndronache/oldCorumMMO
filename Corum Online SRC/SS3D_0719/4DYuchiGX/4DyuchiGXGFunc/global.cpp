...
//	pViewVolume->To			=	*pv3To;
	pViewVolume->fFar		=	fFar;
//	pViewVolume->fFov		=	width;
...
//	v3Up	=	v3Up * ( VECTOR3Length(&v3Right) * (1/fHVRatio));
	...
	VECTOR3		T1, T2, T3, T4;	// 각각 뷰볼륨의 오른쪽위, 왼쪽위, 왼쪽아래, 오른쪽아래.

//	VECTOR3	tpr = *pv3To + v3Right;
//	VECTOR3	tmr = *pv3To - v3Right;
	VECTOR3	tpr,tmr;
	
	VECTOR3_ADD_VECTOR3(&tpr,pv3To,&v3Right);
	VECTOR3_SUB_VECTOR3(&tmr,pv3To,&v3Right);
...
lb_loop_bones_num:
		...
		; 여기서부터 노멀계산..
//		movups		xmm0,[eax+NORMAL_OFFSET_IN_BONE]		; src normal
		; LOAD VECTOR
