...
COLLISION_MODEL_DESC* CoStaticModel::CreateCollisionDesc(CMeshFlag /*gxm*/)
{
	CleanColMesh();
...
BOOL CoStaticModel::CommitIDIMeshObject(CMeshObject* pObj)
{
	...
	LinktToSortLink(&m_pDevAllocObjHead,&m_pDevAllocObjTail,&pObj->m_sortLink);
	bResult = TRUE;
lb_return:
	return bResult;
}
...
BOOL __stdcall CoStaticModel::WriteFile(char* szFileName)
{
	...
	RemoveExt(szNewLMFileName,MAX_NAME_LEN,szFileName);
...
BOOL __stdcall CoStaticModel::ReadFile(char* szFileName,LOAD_CALLBACK_DESC* pCallbackDesc,DWORD dwFlag)
{
	...
	RemoveExt(szRCFileName,MAX_NAME_LEN,szFileName);
	...
	RemoveExt(szColSTMFileName,MAX_NAME_LEN,szFileName);
...
DWORD __stdcall	CoStaticModel::GetMeshObjectWithScreenCoord(
	VECTOR3* pv3IntersectPoint,
	float* pfDist,
	DWORD* pdwFaceGroupIndex,
	POINT* ptCursor,
