...
void SetFaceNormalToVertex(VECTOR3* pv3NormalResult,VECTOR3* pv3World,DWORD /*dwVertexNum*/,WORD* pIndex,DWORD dwFacesNum)
{
	VECTOR3			u,v,c;
	DWORD			i,k;
	...
	for (i=0; i<dwFacesNum; i++)
	{
		...
		VECTOR3_SUB_VECTOR3(&u,&pv3World[pIndex[i*3+1]],&pv3World[pIndex[i*3+0]]);
		VECTOR3_SUB_VECTOR3(&v,&pv3World[pIndex[i*3+2]],&pv3World[pIndex[i*3+0]]);
		CrossProduct(&c,&u,&v);
		for (k=0; k<3; k++)
		{
...
DWORD GetSuffixStringFromExt(char* szDest, char* szFileName,DWORD dwLenFromRight)
{
	...
	DWORD	dwResult = 0;
	DWORD dwLen = RemoveExt(strBuffer,MAX_PATH, szFileName);
	DWORD i,dwGetLen;
