...
//	dwMtlSetIndex = m_dwDefaultMtlSetIndex;
	...
	if (!(dwFlag & RENDER_TYPE_AS_LOD_SUBMODEL))
	{
		...
		{
			// 카메라와의 거리를 설정..
			MODEL_STATUS*	pModelStatus = NULL;
...
lb_render_submodel:	
	...
	DWORD dwDynamicLightNum,dwStaticLightNum;

	dwDynamicLightNum = m_pGeometry->GetDynamicRTLightNum();
	dwStaticLightNum = m_pGeometry->GetStaticRTLightNum();
...
//	if (memcmp(&args,pPrvArgs,sizeof(SET_FRAME_ARGS)-sizeof(DWORD)))
	...
	if (memcmp(&m_matBillboardPrv,m_pGeometry->GetBillBoardMatrix(),sizeof(MATRIX4)))
	{
		...
		m_matBillboardPrv = *m_pGeometry->GetBillBoardMatrix();
	}
...
void __stdcall CoModel::SetBeginIllusionEffect(
	DWORD dwRefIndex,
	DWORD /*dwObjIndex*/
	)
{
...
void __stdcall CoModel::SetEndIllusionEffect(
	DWORD dwRefIndex,
	DWORD /*dwObjIndex*/
	)
{
...
BOOL CoModel::DuplicateStatus(CoModel* /*pDestModel*/,DWORD /*dwFlag*/,DWORD /*dwSrcRefIndex*/)
{
...
//	fclose(fp);
	...
	{
		// lod로드..
		char	suffix[PID_LEN+1];
		DWORD	dwSuffixLen;

		
		dwSuffixLen = GetSuffixStringFromExt(suffix,szFileName,PID_LEN);
		if (dwSuffixLen)
		{
...
void CoModel::ReadLODModels(char* szFileName,DWORD dwSubModelNum)
{
	...
	{
		dwLen = RemoveExt(szLodFileName,MAX_NAME_LEN,szFileName);
