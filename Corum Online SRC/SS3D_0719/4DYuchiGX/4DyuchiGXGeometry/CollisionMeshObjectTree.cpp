...
//	dwTemp;// 새 평면의 갯수.
	...
	// 아웃풋 버퍼.
	if( m_dwOutputBufferCount > 255)		// 일반적인 상식 수준을 넘어가면
		_asm int 3;							// 정지.

	CreateFindBuffer( 2);			// 이때 2는 그냥 테스트값이다.
...
/*
	...
	// 출력 버퍼 지운다.
	if( m_pdwOutputIndexBuffer)
	{
		delete[]	m_pdwOutputIndexBuffer;
		m_pdwOutputIndexBuffer	=	0;
	}
*/
...
{
#ifdef	_DEBUG
	if( dwOutputBufferIndex > m_dwOutputBufferCount)
		_asm int 3;
#endif
