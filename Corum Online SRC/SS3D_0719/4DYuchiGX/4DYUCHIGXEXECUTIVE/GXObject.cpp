...
BOOL CoGXObject::RequestAttach(CoGXObject* pFromObj,char* szObjName)
{
	BOOL		bResult = FALSE;
	DWORD		dwIndex = 0xffffffff;	
...
void CoGXObject::EstimatedMoveForward(VECTOR3* pv3Pos,float fDistance)
{
	...
	this->TryMoveTo( &CollTestResult, &To);
...
void CoGXObject::TryMoveTo( COLLISION_TEST_RESULT* pResult, VECTOR3* pTo)
{
	// 중력 가속도 적용.?
	// v = v0 + a * t;
	...
	COLLISION_TEST_RESULT		CollTestResult;
	m_pCollisionTestFn( m_pExecutive, this, &CollTestResult, pTo, 0);
