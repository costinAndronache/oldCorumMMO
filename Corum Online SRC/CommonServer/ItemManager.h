...
#pragma pack(push,1)
struct COMMONSERVERDECLSPECIFIER ITEM_OPTION_SERVER
{
	DWORD	dwID;
	char	szItemName[50];
...
struct COMMONSERVERDECLSPECIFIER ITEM_EVENT
{
	DWORD	dwid;
	WORD	witem_id;
...
typedef struct COMMONSERVERDECLSPECIFIER SET_ITEM_INFO
{
	WORD				wSetItemID;
...
typedef struct COMMONSERVERDECLSPECIFIER ITEM_MAKING_INFO
{
	WORD				wID;
	WORD				wItem_id1;
	WORD				wItem_id2;
...
struct COMMONSERVERDECLSPECIFIER ITEM_UPGRADE
{
	DWORD	dwID;
	BYTE	bUpgradeLevel;
	BYTE	bCode;
...
typedef struct COMMONSERVERDECLSPECIFIER ITEM_COUNT
{
	WORD	wBaseItemID;		
	BYTE	bServerSet;
	DWORD	dwDungeonID;
...
typedef struct COMMONSERVERDECLSPECIFIER ITEM_LOG
{
	ITEM_SERIAL			Serial;
	BYTE				bLogCode;
	BYTE				bServerSet;
...
typedef struct COMMONSERVERDECLSPECIFIER KARZ_LOG
{
	ITEM_SERIAL			Serial;
	BYTE				bLogCode;
	BYTE				bServerSet;
...
typedef struct COMMONSERVERDECLSPECIFIER SPEEDHACK_LOG
{
	char				szName[20];
	BYTE				bServerSet;
	DWORD				dwDungeonID;
...
typedef struct COMMONSERVERDECLSPECIFIER INVALID_LOG_ITEM
{
	DWORD dwBaseItemID;
}* LPINVALID_LOG_ITEM;


struct COMMONSERVERDECLSPECIFIER USER_SET_ITEM_INFO
{
	DWORD	dwSetID;		// SetID.
...
typedef struct COMMONSERVERDECLSPECIFIER ITEM_STORENATIVE
{
	DWORD	dwID;
	WORD	wItemId;
	BYTE	byType;	
...
struct COMMONSERVERDECLSPECIFIER ITEM_CREATE
{
	void*				DelPos;
	VECTOR2				v2ItemPos;
...
struct COMMONSERVERDECLSPECIFIER ITEM_SERVER
{
