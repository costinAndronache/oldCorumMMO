...
time_t GetDueDay(DWORD nEndMin)
{
	struct tm *pDue;
	time_t now ={0,};
	...
	{
		MessageBox( NULL, "Incorrect Guardian RDay!","Error",0);
	}
	return now;	
}

struct tm SubTm(struct tm &rOrg,struct tm &rTgt)
{
	struct tm tmRet;
	tmRet.tm_sec	= rOrg.tm_sec - rTgt.tm_sec;
	tmRet.tm_min	= rOrg.tm_min - rTgt.tm_min;
	tmRet.tm_hour	= rOrg.tm_hour - rTgt.tm_hour;
	tmRet.tm_mday	= rOrg.tm_mday - rTgt.tm_mday;
	tmRet.tm_mon	= rOrg.tm_mon - rTgt.tm_mon;
	tmRet.tm_year	= rOrg.tm_year - rTgt.tm_year;
	tmRet.tm_wday	= rOrg.tm_wday - rTgt.tm_wday;
	tmRet.tm_yday	= rOrg.tm_yday - rTgt.tm_yday;
	tmRet.tm_isdst	= rOrg.tm_isdst - rTgt.tm_isdst;
	return tmRet;
}

int GetReserveDay(DWORD dwDueDay)
{
	time_t now = {0,};
	time(&now);
	...
	struct tm Due;
{
	...
	struct tm* pDue	= localtime((time_t*)&dwDueDay);
	if (pDue)
	{
		Due = *pDue;
	}
	...
	struct tm Ret = SubTm(Due,Now);

	return (Ret.tm_year * 365) + Ret.tm_yday;
}
...
//kjk
	memset(m_MEMBER_DB_NAME,0,sizeof(m_MEMBER_DB_NAME));		
	memset(m_MEMBER_DB_ACCOUNT,0,sizeof(m_MEMBER_DB_ACCOUNT));		
	memset(m_MEMBER_DB_PASSWORD,0,sizeof(m_MEMBER_DB_PASSWORD));	
	memset(m_MEMBER_DB_ADDRESS,0,sizeof(m_MEMBER_DB_ADDRESS));
	memset(m_MEMBER_NET_DB_NAME,0,sizeof(m_MEMBER_NET_DB_NAME));		
	memset(m_MEMBER_NET_DB_ACCOUNT,0,sizeof(m_MEMBER_NET_DB_ACCOUNT));		
	memset(m_MEMBER_NET_DB_PASSWORD,0,sizeof(m_MEMBER_NET_DB_PASSWORD));	
	memset(m_MEMBER_NET_DB_ADDRESS,0,sizeof(m_MEMBER_NET_DB_ADDRESS));
...
//kjk
//	memset(m_CORUM_MEMBER_DB,0,sizeof(m_CORUM_MEMBER_DB));
//	memset(m_CORUM_MEMBER_NETMARBLE_DB,0,sizeof(m_CORUM_MEMBER_NETMARBLE_DB));
...
//		return;
	...
	m_pEdit = new CEdit;
	m_pEdit->Create(WS_CHILD|WS_VISIBLE|WS_TABSTOP|WS_BORDER,CRect(25,55,370,68),this,1);
	m_pEdit->SetReadOnly();
	m_pEdit->SetFocus();
...
//	LoadItemData();	//hwoarang

	CString	strMsg ="Ready";
	m_pEdit->SetWindowText(strMsg);
}
...
//	pView->TestSeverInitialize();

//	pView->GuardianInitialize();

//	pView->GuardianLevelInitialize();

//	pView->GuardianSkillInitialize();		// 구가디언 스킬추가

//	pView->ConvertNewItem("_origin");				// 신아이템 구조로 변환
...
//------------------------------------------------------------------------------
void CCopyItemDelete::CharacterMoveSin2()
{
	char				szQuery[0xff]	= {0,};
	char				Name[20]		= {0,};
	DBRECEIVEDATA*		pData			= NULL;
...
//	folder.Format("%s\\CharacterInfo_MoveLogData",g_szExePath);
...
//	sprintf(szQuery,"Select * From CharacterMoveSin");		
	...
	for( i = 0; i < pData->Query.select.dwRowCount; i++ )	
	{
		...
		int nNameSize = strlen(CharacterMove.szCharName);

		if(nNameSize>14)
		{
			fprintf(pFile,"[Server:%d][ID:%s][Name:%s]이름이 길다..\n",CharacterMove.byServerSet,CharacterMove.szID,CharacterMove.szCharName);
			continue;
		}

		sprintf(szName, "%s55", CharacterMove.szCharName);

		// 1. 이전할 서버(에스메랄드,펠트로스)에 같은 캐릭명이 존재하는지
		sprintf(szQuery,"Select [Name] From CharacterInfo Where [Name] = '%s'", szName);
		memset(Name,0,sizeof(Name));
		...
		else 
		{
			...
			if(bNum)
			{
				...
				sprintf(szQuery,"CZP_CREATE_NEW_CHARACTER '%s','%s',%d,%d,%d,%d,'%s'",CharacterMove.szID, szName, wCharNum,ItemChrMove.wHead,ItemChrMove.wClass,1,szIp);
				...
				switch(Result.dwResult)
				{
					...
					case 1:
					{
						...
						sprintf(szQuery, "Update CharacterInfo Set Point=?, Point_Skill=?, Exp=?, [Level]=?, Honor=?, Luck=?, Money=?, Equip=?, Inv_large=?, Inv_small=?, Belt=?, Guardian=? Where Name = '%s'", szName);
...
//										CharMoveUpdateDB.cInvLarge[i].m_Serial.bServerSet, CharMoveUpdateDB.cInvLarge[i].m_Serial.bServerSerialID, CharMoveUpdateDB.cInvLarge[i].m_Serial.wRestartCount, CharMoveUpdateDB.cInvLarge[i].m_Serial.dwSerialNum);
									...
									sprintf(szQuery, "Update CharacterInfo Set Inv_large=? Where Name = '%s'", szName);
...
//											CharMoveUpdateDB.cGuardian[i].m_Serial.bServerSet, CharMoveUpdateDB.cGuardian[i].m_Serial.bServerSerialID, CharMoveUpdateDB.cGuardian[i].m_Serial.wRestartCount, CharMoveUpdateDB.cGuardian[i].m_Serial.dwSerialNum);
	...
	sprintf(szQuery,"Delete From Characterinfo_MoveLogData");
...
void CCopyItemDelete::CharacterMoveSin()
{
	...
	CString	folder;
	folder.Format("%s\\CharacterMoveSin",g_szExePath);
//	folder.Format("%s\\CharacterMoveSinTest",g_szExePath);
	...
	sprintf(szQuery,"Select Count(*) From CharacterInfo_MoveLog");
//	sprintf(szQuery,"Select Count(*) From CharacterInfo_MoveLogData");
	ret = m_pDb->OpenRecord(szQuery,&dwCount,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[sel]);
	sprintf(szQuery,"Select * From CharacterInfo_MoveLog");
//	sprintf(szQuery,"Select * From CharacterInfo_MoveLogData");
	pData = m_pDb->OpenRecordEx(szQuery,dwCount,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[sel]);
	...
	for( i = 0; i < pData->Query.select.dwRowCount; i++ )
	{
		...
		sprintf(szName, "%s", CharacterMove.szCharName);//kjk
		if(ret >= 1)
		{
			// 동일명 캐릭터가 있습니다.
			sprintf(szName, "%s55", CharacterMove.szCharName);//kjk
//			sprintf(szName, "천상추혼25식");//kjk
			sprintf(szQuery,"Select [Name] From CharacterInfo Where [Name] = '%s'",szName);
			ret = m_pDb->OpenRecord(szQuery,Name,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[sel]);
			if( ret >= 1 )
			{
				fprintf(pFile,"[Server:%d][ID:%s][Name:%s]같은 이름이 있다..\n",CharacterMove.byServerSet,CharacterMove.szID,CharacterMove.szCharName);
				continue;
			}
			int nNameSize = strlen(szName);
			if(nNameSize>16)
			{
				fprintf(pFile,"[Server:%d][ID:%s][Name:%s]이름이 길다..\n",CharacterMove.byServerSet,CharacterMove.szID,CharacterMove.szCharName);
				continue;
		...
		{
			sprintf(szQuery,"Select [Name] From DeletedCharacter Where [Name] = '%s'",CharacterMove.szCharName);
			memset(Name,0,sizeof(Name));
			ret = m_pDb->OpenRecord(szQuery,Name,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[sel]);
			if(ret >= 1)
			{
...
//		sprintf(szName, "천상추혼25식");//kjk
		...
		if(CharacterMove.byServerSet == 1)
			ret = m_pDb->OpenRecord(szQuery,Name,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[0]);
		else if(CharacterMove.byServerSet == 2)//자이피
			ret = m_pDb->OpenRecord(szQuery,Name,DEFAULT_ROWS_PER_READ,m_CORUM_OLD2_DB);
		else if(CharacterMove.byServerSet == 3)//이슈리아
			ret = m_pDb->OpenRecord(szQuery,Name,DEFAULT_ROWS_PER_READ,m_CORUM_OLD3_DB);
		else if(CharacterMove.byServerSet == 4)
			ret = m_pDb->OpenRecord(szQuery,Name,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[1]);
		else if(CharacterMove.byServerSet == 5)
			ret = m_pDb->OpenRecord(szQuery,Name,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[2]);
		...
		if(CharacterMove.byServerSet == 1)
			...
			ret = m_pDb->OpenRecord(szQuery,&ItemChrMove,DEFAULT_ROWS_PER_READ,m_CORUM_OLD2_DB);
		else if(CharacterMove.byServerSet == 3)//이슈리아
			ret = m_pDb->OpenRecord(szQuery,&ItemChrMove,DEFAULT_ROWS_PER_READ,m_CORUM_OLD3_DB);
		else if(CharacterMove.byServerSet == 4)
			ret = m_pDb->OpenRecord(szQuery,&ItemChrMove,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[1]);
		else if(CharacterMove.byServerSet == 5)
			ret = m_pDb->OpenRecord(szQuery,&ItemChrMove,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[2]);
		...
		else 
		{
			...
			WORD wCharNum	= 0;
			BOOL bNum		= FALSE;
//			memset( &CharacterMove.szID, 0, sizeof( CharacterMove.szID ) );
//			sprintf(CharacterMove.szID, "hon21c");
...
//				sprintf(szQuery,"CZP_CREATE_NEW_CHARACTER '%s','%s',%d,%d,%d,%d,'%s'",CharacterMove.szID,szName,wCharNum,ItemChrMove.wHead,ItemChrMove.wClass,1,szIp);
				...
				switch(Result.dwResult)
				{
					...
					case 1:
					{
						...
						sprintf(szQuery, "Update CharacterInfo Set Point=?, Point_Skill=?, Exp=?, [Level]=?, Honor=?, Luck=?, Money=?, Equip=?, Inv_large=?, Inv_small=?, Belt=?, Guardian=? Where Name = '%s'",szName);//CharacterMove.szCharName);
						...
						int result  = g_pDb->ExecuteSQLByParam(szQuery,pBinding,&CharMoveUpdateDB,nColumn,m_CORUM_GAME_DB[sel]);	
						...
						for(i = 0; i < MAX_INV_LARGE; i++)
						{
							if(CharMoveUpdateDB.cInvLarge[i].GetID()/ITEM_DISTRIBUTE==ITEM_GUARDIAN_INDEX)
							{
								...
								else
								{
									...
									sprintf(szQuery, "Update CharacterInfo Set Inv_large=? Where Name = '%s'", szName);//CharacterMove.szCharName);
						...
						for(i = 0; i < MAX_INV_GUARDIAN; i++)
						{
							...
							{					
								...
								else
								{
									...
									sprintf(szQuery, "Update CharacterInfo Set Guardian=? Where Name = '%s'", szName);//CharacterMove.szCharName);
							...
							else if(CharacterMove.byServerSet == 2)
							{
								ret = m_pDb->OpenRecord(szQuery,&DeleteResult,1,m_CORUM_OLD2_DB);
								ServerSet	=	12;
							}
							else if(CharacterMove.byServerSet == 3)
							{
								ret = m_pDb->OpenRecord(szQuery,&DeleteResult,1,m_CORUM_OLD3_DB);
								ServerSet	=	12;
							}
							else if(CharacterMove.byServerSet == 4)
							{
							...
							if( ServerSet != 0 )
							{
								sprintf(szQuery, "CZP_INSERT_NAME_FOR_WEB_BOARD %d, '%s', '%s', %d", 
									ServerSet, CharacterMove.szID, szName, ItemChrMove.wClass );
								if( ServerSet == 12)
								{
									ret = m_pDb->ExecuteSQL(szQuery, (BYTE)m_CORUM_MEMBER_DB);
								}
								else if( ServerSet == 16)
								{
									ret = m_pDb->ExecuteSQL(szQuery, (BYTE)m_CORUM_MEMBER_NETMARBLE_DB);
								}
							}
...
//kjk
	sprintf(keyname,"MEMBERDB_INFO");
	::GetPrivateProfileString(keyname,"CORUM_DB_NAME",		"",m_MEMBER_DB_NAME			,sizeof(m_MEMBER_DB_NAME)		,path);	
	::GetPrivateProfileString(keyname,"CORUM_DB_ACCOUNT",	"",m_MEMBER_DB_ACCOUNT		,sizeof(m_MEMBER_DB_ACCOUNT)	,path);	
	::GetPrivateProfileString(keyname,"CORUM_DB_PASSWORD",	"",m_MEMBER_DB_PASSWORD		,sizeof(m_MEMBER_DB_PASSWORD)	,path);
	::GetPrivateProfileString(keyname,"CORUM_DB_ADDRESS",	"",m_MEMBER_DB_ADDRESS		,sizeof(m_MEMBER_DB_ADDRESS)	,path);

	sprintf(keyname,"MEMBERDB_NETMARBLE_INFO");
	::GetPrivateProfileString(keyname,"CORUM_DB_NAME",		"",m_MEMBER_NET_DB_NAME			,sizeof(m_MEMBER_NET_DB_NAME)		,path);	
	::GetPrivateProfileString(keyname,"CORUM_DB_ACCOUNT",	"",m_MEMBER_NET_DB_ACCOUNT		,sizeof(m_MEMBER_NET_DB_ACCOUNT)	,path);	
	::GetPrivateProfileString(keyname,"CORUM_DB_PASSWORD",	"",m_MEMBER_NET_DB_PASSWORD		,sizeof(m_MEMBER_NET_DB_PASSWORD)	,path);
	::GetPrivateProfileString(keyname,"CORUM_DB_ADDRESS",	"",m_MEMBER_NET_DB_ADDRESS		,sizeof(m_MEMBER_NET_DB_ADDRESS)	,path);
...
void CCopyItemDelete::TestSeverInitialize()
{
	char				szQuery[1000]	= {0,};
	char				Name[20]		= {0,};
	char				ID[20]			= {0,};
...
//		sprintf(Name, "국자");
		...
		sprintf(szQuery,"Select Class,Point,[Level],Ego,Str,[Int],Dex,Vit,LastUsedStatItem,"
						"Point_Skill,SelectedSkill_L,SelectedSkill_R,SkillLevel,LastUsedSkillItem,"
						"Equip,Inv_large,Inv_small,Belt,Guardian,Money From CharacterInfo Where [Name]='%s'",Name);

		memset(&AllInit,0,sizeof(ALLINIT));
		ret = m_pDb->OpenRecord(szQuery,&AllInit,1,m_CORUM_GAME_DB[sel]);
...
//스텟 초기화
//======================================================================================================
		if(AllInit.m_byLastUsedStatItem == 0)
		{
			for(int i = 0; i < AllInit.m_dwLevel-1; i++)
			{
				AllInit.m_wPoint += byStstus[i];
			}
		}
		else if(AllInit.m_byLastUsedStatItem != 0)
		{
			for(int i = 0; i < AllInit.m_dwLevel-1; i++)
			{
				AllInit.m_wPoint += byStstus[i];
			...
			AllInit.m_wPoint = AllInit.m_wPoint + AllInit.m_byLastUsedStatItem - 50;
		}

		switch(AllInit.m_wClass)
		{
		case 1:
			{
				AllInit.m_dwEgo	= 10;
...
//스킬 초기화
//======================================================================================================
		if(AllInit.m_byLastUsedSkillItem == 0)
		{
			AllInit.m_wPointSkill = AllInit.m_dwLevel;
		}
		else if(AllInit.m_byLastUsedSkillItem != 0)
		{
			AllInit.m_wPointSkill = AllInit.m_dwLevel + AllInit.m_byLastUsedSkillItem - 50;
		}
		
		AllInit.m_shSelectedSkill[0] = 151;
		AllInit.m_shSelectedSkill[1] = -1;
		
		memset(AllInit.m_pwSkillLevel,0,sizeof(AllInit.m_pwSkillLevel));
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//======================================================================================================
		...
		for( int equip = 0; equip < MAX_EQUIP; equip++ )
		{
			if(AllInit.Equip[equip].GetID() != 0)
			{	
				CBaseItem* pBaseItem = NULL;	
				pBaseItem = g_pBaseItemHash->GetData(AllInit.Equip[equip].GetID());
				
				if(pBaseItem == NULL)
				{
					memset(&AllInit.Equip[equip],0,sizeof(CItem2));
					bIsUpdate = TRUE;
				}
				else
				{
					int nValue = AllInit.Equip[equip].m_wItemID/ITEM_DISTRIBUTE;
					
					if(nValue>=ITEM_WEAPONST_INDEX && nValue<=ITEM_WEAPONEND_INDEX)
					{
						if(AllInit.Equip[equip].m_ItemWeapon.wMana_Damage >= 1000)
						{
							memset(&AllInit.Equip[equip],0,sizeof(CItem2));
							bIsUpdate = TRUE;
						}
						else if(AllInit.Equip[equip].m_ItemWeapon.wMin_Damage == 0 ||
							AllInit.Equip[equip].m_ItemWeapon.wMax_Damage == 0)
						{
							memset(&AllInit.Equip[equip],0,sizeof(CItem2));
							bIsUpdate = TRUE;
						}
					}
					else if(nValue>=ITEM_CLASS_HELMET && nValue<=ITEM_CLASS_BOOTS)
					{
						if(	AllInit.Equip[equip].m_Item_Armor.wDefense >= 1000 ||
							AllInit.Equip[equip].m_Item_Armor.wDefense	== 0)
						{
							memset(&AllInit.Equip[equip],0,sizeof(CItem2));
							bIsUpdate = TRUE;
						}
		...
		for( int inv_large = 0; inv_large < MAX_INV_LARGE; inv_large++ )
		{
			if(AllInit.Inv_Large[inv_large].GetID() != 0)
			{	
				CBaseItem* pBaseItem = NULL;	
				pBaseItem = g_pBaseItemHash->GetData(AllInit.Inv_Large[inv_large].GetID());
				
				if(pBaseItem == NULL)
				{
					memset(&AllInit.Inv_Large[inv_large],0,sizeof(CItem2));
					bIsUpdate = TRUE;
				}
				else
				{
					int nValue = AllInit.Inv_Large[inv_large].m_wItemID/ITEM_DISTRIBUTE;
					
					if(nValue>=ITEM_WEAPONST_INDEX && nValue<=ITEM_WEAPONEND_INDEX)
					{
						if(AllInit.Inv_Large[inv_large].m_ItemWeapon.wMana_Damage >= 1000)
						{
							memset(&AllInit.Inv_Large[inv_large],0,sizeof(CItem2));
							bIsUpdate = TRUE;
						}
						else if(AllInit.Inv_Large[inv_large].m_ItemWeapon.wMin_Damage == 0 ||
							AllInit.Inv_Large[inv_large].m_ItemWeapon.wMax_Damage == 0)
						{
							memset(&AllInit.Inv_Large[inv_large],0,sizeof(CItem2));
							bIsUpdate = TRUE;
						}
					...
					{
						if(	AllInit.Inv_Large[inv_large].m_Item_Armor.wDefense >= 1000 ||
							AllInit.Inv_Large[inv_large].m_Item_Armor.wDefense	== 0)
						{
							memset(&AllInit.Inv_Large[inv_large],0,sizeof(CItem2));
							bIsUpdate = TRUE;
						}
		...
		for( int inv_small = 0; inv_small < MAX_INV_SMALL; inv_small++ )
		{
			memset(&AllInit.Inv_Small[inv_small],0,sizeof(CItem2));
		}
		// Belt
		for( int belt = 0; belt < MAX_BELT; belt++ )
		{
			memset(&AllInit.Belt[belt],0,sizeof(CItem2));
		}
				
		// Guardian
		for( int guardian = 0; guardian < MAX_GUARDIAN; guardian++ )
		{
			if(AllInit.Guardian[guardian].GetID() != 0)
			{	
				CBaseItem* pBaseItem = NULL;	
				pBaseItem = g_pBaseItemHash->GetData(AllInit.Guardian[guardian].GetID());
				
				if(pBaseItem == NULL)
				{
					memset(&AllInit.Guardian[guardian],0,sizeof(CItem2));
					bIsUpdate = TRUE;
				}
			}
		}
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		...
		int			nColumn			= 20;	// 파라미터 갯수 지정 
		DWORD		dwParamOffset	= 0;	// 파라미터 Binding 정보 구조체에서 각 파라미터 Value들의 Offset
		DBBINDING*	pBinding		= m_pDb->CreateParamInfo(nColumn);		
		if(!pBinding)	continue;
		
		sprintf(szQuery,"Update CharacterInfo Set [Class]=?, [Point]=?, [Level]=?, [Ego]=?, [Str]=?, [Int]=?, [Dex]=?, [Vit]=?, [LastUsedStatItem]=?, "
						"[Point_Skill]=?, [SelectedSkill_L]=?, [SelectedSkill_R]=?, [SkillLevel]=?, [LastUsedSkillItem]=?, "
						"[Equip]=?, [Inv_large]=?, [Inv_small]=?, [Belt]=?, [Guardian]=?, [Money]=? Where [Name]='%s' ",Name);
		
		pBinding[0].obValue		= dwParamOffset;	
		pBinding[0].cbMaxLen	= sizeof(AllInit.m_wClass);
		pBinding[0].wType		= DBTYPE_I2;
		dwParamOffset			+= sizeof(AllInit.m_wClass);				
		
		pBinding[1].obValue		= dwParamOffset;	
		pBinding[1].cbMaxLen	= sizeof(AllInit.m_wPoint);
		pBinding[1].wType		= DBTYPE_I2;
		dwParamOffset			+= sizeof(AllInit.m_wPoint);		
		
		pBinding[2].obValue		= dwParamOffset;	
		pBinding[2].cbMaxLen	= sizeof(AllInit.m_dwLevel);
		pBinding[2].wType		= DBTYPE_I4;
		dwParamOffset			+= sizeof(AllInit.m_dwLevel);			
		
		pBinding[3].obValue		= dwParamOffset;	
		pBinding[3].cbMaxLen	= sizeof(AllInit.m_dwEgo);
		pBinding[3].wType		= DBTYPE_I4;
		dwParamOffset			+= sizeof(AllInit.m_dwEgo);
		
		pBinding[4].obValue		= dwParamOffset;	
		pBinding[4].cbMaxLen	= sizeof(AllInit.m_dwStr);
		pBinding[4].wType		= DBTYPE_I4;
		dwParamOffset			+= sizeof(AllInit.m_dwStr);
		
		pBinding[5].obValue		= dwParamOffset;	
		pBinding[5].cbMaxLen	= sizeof(AllInit.m_dwInt);
		pBinding[5].wType		= DBTYPE_I4;
		dwParamOffset			+= sizeof(AllInit.m_dwInt);

		pBinding[6].obValue		= dwParamOffset;	
		pBinding[6].cbMaxLen	= sizeof(AllInit.m_dwDex);
		pBinding[6].wType		= DBTYPE_I4;
		dwParamOffset			+= sizeof(AllInit.m_dwDex);

		pBinding[7].obValue		= dwParamOffset;	
		pBinding[7].cbMaxLen	= sizeof(AllInit.m_dwVit);
		pBinding[7].wType		= DBTYPE_I4;
		dwParamOffset			+= sizeof(AllInit.m_dwVit);

		pBinding[8].obValue		= dwParamOffset;	
		pBinding[8].cbMaxLen	= sizeof(AllInit.m_byLastUsedStatItem);
		pBinding[8].wType		= DBTYPE_UI1;
		dwParamOffset			+= sizeof(AllInit.m_byLastUsedStatItem);

		pBinding[9].obValue		= dwParamOffset;	
		pBinding[9].cbMaxLen	= sizeof(AllInit.m_wPointSkill);
		pBinding[9].wType		= DBTYPE_I2;
		dwParamOffset			+= sizeof(AllInit.m_wPointSkill);				
		
		pBinding[10].obValue		= dwParamOffset;	
		pBinding[10].cbMaxLen	= sizeof(AllInit.m_shSelectedSkill[0]);
		pBinding[10].wType		= DBTYPE_I2;
		dwParamOffset			+= sizeof(AllInit.m_shSelectedSkill[0]);			
		
		pBinding[11].obValue		= dwParamOffset;	
		pBinding[11].cbMaxLen	= sizeof(AllInit.m_shSelectedSkill[1]);
		pBinding[11].wType		= DBTYPE_I2;
		dwParamOffset			+= sizeof(AllInit.m_shSelectedSkill[1]);
		
		pBinding[12].obValue		= dwParamOffset;	
		pBinding[12].cbMaxLen	= sizeof(AllInit.m_pwSkillLevel);
		pBinding[12].wType		= DBTYPE_BYTES;
		dwParamOffset			+= sizeof(AllInit.m_pwSkillLevel);
		
		pBinding[13].obValue		= dwParamOffset;	
		pBinding[13].cbMaxLen	= sizeof(AllInit.m_byLastUsedSkillItem);
		pBinding[13].wType		= DBTYPE_UI1;
		dwParamOffset			+= sizeof(AllInit.m_byLastUsedSkillItem);

		pBinding[14].obValue		= dwParamOffset;	
		pBinding[14].cbMaxLen	= sizeof(AllInit.Equip);
		pBinding[14].wType		= DBTYPE_BYTES;
		dwParamOffset			+= sizeof(AllInit.Equip);				
		
		pBinding[15].obValue		= dwParamOffset;	
		pBinding[15].cbMaxLen	= sizeof(AllInit.Inv_Large);
		pBinding[15].wType		= DBTYPE_BYTES;
		dwParamOffset			+= sizeof(AllInit.Inv_Large);		
		
		pBinding[16].obValue		= dwParamOffset;	
		pBinding[16].cbMaxLen	= sizeof(AllInit.Inv_Small);
		pBinding[16].wType		= DBTYPE_BYTES;
		dwParamOffset			+= sizeof(AllInit.Inv_Small);			
		
		pBinding[17].obValue		= dwParamOffset;	
		pBinding[17].cbMaxLen	= sizeof(AllInit.Belt);
		pBinding[17].wType		= DBTYPE_BYTES;
		dwParamOffset			+= sizeof(AllInit.Belt);
		
		pBinding[18].obValue		= dwParamOffset;	
		pBinding[18].cbMaxLen	= sizeof(AllInit.Guardian);
		pBinding[18].wType		= DBTYPE_BYTES;
		dwParamOffset			+= sizeof(AllInit.Guardian);

		pBinding[19].obValue		= dwParamOffset;	
		pBinding[19].cbMaxLen	= sizeof(AllInit.dMoney);
		pBinding[19].wType		= DBTYPE_I4;
		dwParamOffset			+= sizeof(AllInit.dMoney);
		
		m_pDb->ExecuteSQLByParam(szQuery,pBinding,&AllInit,nColumn,m_CORUM_GAME_DB[sel]);
		m_pDb->ReleaseParamInfo(pBinding);
	}
...
//			sprintf(ID, "hon21c");
			...
			for( int banklarge = 0; banklarge < MAX_BANK_LARGE; banklarge++ )
			{
				if(BankInfo.Bank_Large[banklarge].GetID() != 0)
				{	
					CBaseItem* pBaseItem = NULL;	
					pBaseItem = g_pBaseItemHash->GetData(BankInfo.Bank_Large[banklarge].GetID());
					...
					{
						int nValue = BankInfo.Bank_Large[banklarge].m_wItemID/ITEM_DISTRIBUTE;
						
						if(nValue>=ITEM_WEAPONST_INDEX && nValue<=ITEM_WEAPONEND_INDEX)
						{
							if(BankInfo.Bank_Large[banklarge].m_ItemWeapon.wMana_Damage >= 1000)
							{
								memset(&BankInfo.Bank_Large[banklarge],0,sizeof(CItem2));
							}
							else if(BankInfo.Bank_Large[banklarge].m_ItemWeapon.wMin_Damage == 0 ||
								BankInfo.Bank_Large[banklarge].m_ItemWeapon.wMax_Damage == 0)
							{
								memset(&BankInfo.Bank_Large[banklarge],0,sizeof(CItem2));
							}
						}
						...
						{
							if(	BankInfo.Bank_Large[banklarge].m_Item_Armor.wDefense >= 1000 ||
								BankInfo.Bank_Large[banklarge].m_Item_Armor.wDefense	== 0)
							{
								memset(&BankInfo.Bank_Large[banklarge],0,sizeof(CItem2));
							}
						}
			...
			sprintf(szQuery,"Update Bankinfo Set Bank_Large=?,Bank_Small=?,[Money]=? Where [ID]='%s'",ID);
			DBBINDING* pBinding = m_pDb->CreateParamInfo(nColumn);				
			if(!pBinding) continue;
	...
	DWORD		dwPlayerShopCount	= 0;
	DWORD		dwCharacterIndex	= 0;
	PLAYERSHOP	PlayerShopInfo;
	ShowQuery.Format("All PlayerShop Query");
	m_pEdit->SetWindowText(ShowQuery);
	
	sprintf(szQuery,"SELECT Count(*) FROM PlayerShop");	
	ret = m_pDb->OpenRecord(szQuery,&dwPlayerShopCount,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[sel]);
	sprintf(szQuery,"SELECT [Char_Index] FROM PlayerShop");
	pData = m_pDb->OpenRecordEx(szQuery,dwPlayerShopCount,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[sel]);
	...
	if( pData != NULL )
	{
		for( DWORD i = 0; i < pData->Query.select.dwRowCount; i++ )
		{
			...
			memcpy(	(DWORD*)&dwCharacterIndex,pData->Query.select.pResult + ( i * pData->Query.select.dwRowSize ),pData->Query.select.dwRowSize);
			
			ShowQuery.Format("Q(Bank):<%d>[%d].[%d]",pData->Query.select.dwRowCount,i+1,dwCharacterIndex);
			m_pEdit->SetWindowText(ShowQuery);
			
			sprintf(szQuery,"SELECT Item1, Item2, Item3, Item4, Item5, Item6, Item7, Item8, Item9, Item10 FROM PlayerShop WHERE [Char_Index]= %d",dwCharacterIndex);
			memset(&PlayerShopInfo,0,sizeof(PLAYERSHOP));
			int ret = m_pDb->OpenRecord(szQuery,&PlayerShopInfo,1,m_CORUM_GAME_DB[sel]);
			if( ret <= 0 )	continue;

			for(int j = 0; j < 10; j++)
			{
				if(PlayerShopInfo.m_Item[j].GetID() != 0)
				{
					if( PlayerShopInfo.m_Item[j].GetKind() == __ITEM_WEAPON__ || 
						PlayerShopInfo.m_Item[j].GetKind() == __ITEM_AROMR__  ||
						PlayerShopInfo.m_Item[j].GetKind() == __ITEM_GUARDIAN__ )
					{
						bFindFuck	=	FALSE;
					}
					else
					{
						memset(&PlayerShopInfo.m_Item[j],0,sizeof(CItem2));
						continue;
					}

					CBaseItem* pBaseItem = NULL;	
					pBaseItem = g_pBaseItemHash->GetData(PlayerShopInfo.m_Item[j].GetID());
					...
					{
						int nValue = PlayerShopInfo.m_Item[j].m_wItemID/ITEM_DISTRIBUTE;
						
						if(nValue>=ITEM_WEAPONST_INDEX && nValue<=ITEM_WEAPONEND_INDEX)
						{
							if(PlayerShopInfo.m_Item[j].m_ItemWeapon.wMana_Damage >= 1000)
							{
								memset(&PlayerShopInfo.m_Item[j],0,sizeof(CItem2));
							}
							else if(PlayerShopInfo.m_Item[j].m_ItemWeapon.wMin_Damage == 0 ||
								PlayerShopInfo.m_Item[j].m_ItemWeapon.wMax_Damage == 0)
							{
								memset(&PlayerShopInfo.m_Item[j],0,sizeof(CItem2));
							}
						}
						...
						{
							if(	PlayerShopInfo.m_Item[j].m_Item_Armor.wDefense >= 1000 ||
								PlayerShopInfo.m_Item[j].m_Item_Armor.wDefense	== 0)
							{
								memset(&PlayerShopInfo.m_Item[j],0,sizeof(CItem2));
							}
						}
			...
			// 여기서 업데이트 한다. Player Shop
			sprintf(szQuery,"Update PlayerShop Set Item1=?, Item2=?, Item3=?, Item4=?, Item5=?, Item6=?, Item7=?, Item8=?, Item9=?, Item10=? WHERE [Char_Index]= %d", dwCharacterIndex);
			...
			{
				pBinding[i].obValue		= dwParamOffset;	
				pBinding[i].cbMaxLen	= sizeof(CItem2);
				pBinding[i].wType		= DBTYPE_BYTES;
				dwParamOffset			+= sizeof(CItem2);
			}
			
			int affected = m_pDb->ExecuteSQLByParam(szQuery,pBinding,&PlayerShopInfo,10,m_CORUM_GAME_DB[sel]);	
		}
	}
	...
	// 다 끝났다.
	AfxMessageBox("All Initialize End!!!",MB_OK|MB_ICONINFORMATION);
}
...
void LoadItemData()
{
	...
	pData = g_pDb->OpenRecordEx("Select * from Item_Armor", 1000, DEFAULT_ROWS_PER_READ, g_CORUM_GAME_DB[0] );
	for( i=0; i<pData->Query.select.dwRowCount; i++ )
	{
		pBaseItem = new CBaseItem;
		memset( pBaseItem, 0, sizeof(CBaseItem) );
	...
	pData = g_pDb->OpenRecordEx("Select * from Item_Upgrade",500,DEFAULT_ROWS_PER_READ,g_CORUM_GAME_DB[0]);
	for( i=0; i<pData->Query.select.dwRowCount; i++)
	{
		pBaseItem = new CBaseItem;
		memset( pBaseItem, 0, sizeof(CBaseItem) );
	...
	pData = g_pDb->OpenRecordEx("Select * from Item_liquid",500,DEFAULT_ROWS_PER_READ,g_CORUM_GAME_DB[0]);
	for( i=0; i<pData->Query.select.dwRowCount; i++)
	{
		pBaseItem = new CBaseItem;
		memset( pBaseItem, 0, sizeof(CBaseItem) );
	...
	pData = g_pDb->OpenRecordEx("Select * from Item_Edition",500,DEFAULT_ROWS_PER_READ,g_CORUM_GAME_DB[0]);
	for( i=0; i<pData->Query.select.dwRowCount; i++)
	{
		pBaseItem = new CBaseItem;
		memset( pBaseItem, 0, sizeof(CBaseItem) );
...
void CCopyItemDelete::GuardianInitialize()
{
	char				szQuery[1000]	= {0,};
	char				Name[20]		= {0,};
	char				ID[20]			= {0,};
	...
	int					sel				= m_ComBoCtrl.GetCurSel();
	CString				ShowQuery;
	ALLGUARDIANITEM		AllGuardianItem;
	BankItem3			BankInfo;
	DWORD				dwGuardianExp	= 0;
	int					ret				= 0;
	BOOL				bChange			= 0;
	...
	for( DWORD i = 0; i < pData->Query.select.dwRowCount; i++ )
	{
		...
		memset(&AllGuardianItem,0,sizeof(ALLGUARDIANITEM));
		ret = m_pDb->OpenRecord(szQuery,&AllGuardianItem,1,m_CORUM_GAME_DB[sel]);
...
//		CBaseItem* pBaseItem;
		...
		// Inv_Large	라지인벤에서 가디언아이템을 검색한다.
		for( int inv_large = 0; inv_large < MAX_INV_LARGE; inv_large++ )
		{
			WORD id	= AllGuardianItem.Inv_Large[inv_large].GetID();
			if(id == 0)	continue;
			if( id / ITEM_DISTRIBUTE == ITEM_GUARDIAN_INDEX )
			{
				bChange = TRUE;
				AllGuardianItem.Inv_Large[inv_large].m_Item_Guardian.dwEndLife	= 0;
				AllGuardianItem.Inv_Large[inv_large].m_Item_Guardian.wComplete	= 0;
				if( id >= 3900 && id <= 3931 )//가디언 알탱이일때
				{
					AllGuardianItem.Inv_Large[inv_large].m_Item_Guardian.dwID		= 0;
					AllGuardianItem.Inv_Large[inv_large].m_Item_Guardian.bCreate	= 0;
				}
				else
				{
					if( AllGuardianItem.Inv_Large[inv_large].m_Item_Guardian.dwID == 0 )
					{
						continue;
					}
					
					sprintf( szQuery,"Select [exp] From GuardianInfo Where [ID] = %d", AllGuardianItem.Inv_Large[inv_large].m_Item_Guardian.dwID );
					ret = m_pDb->OpenRecord(szQuery,&dwGuardianExp,1,m_CORUM_GAME_DB[sel]);
					
					if( ret > 0 )
					{
						AllGuardianItem.Inv_Large[inv_large].m_Item_Guardian.wComplete	= dwGuardianExp;
						AllGuardianItem.Inv_Large[inv_large].m_Item_Guardian.dwEndLife	= GetDueDay(525600);//pBaseItem->BaseITem_Guardian.dwDueDayTime);
					}
				}
			}
		...
		// Guardian		가디언인벤에서 가디언아이템을 검색한다.
		for( int guardian = 0; guardian < MAX_GUARDIAN; guardian++ )
		{
			WORD id	= AllGuardianItem.Guardian[guardian].GetID();
			if(id == 0)	continue;	
			if( id / ITEM_DISTRIBUTE == ITEM_GUARDIAN_INDEX )
			{
				bChange = TRUE;
				AllGuardianItem.Guardian[guardian].m_Item_Guardian.dwEndLife	= 0;
				AllGuardianItem.Guardian[guardian].m_Item_Guardian.wComplete	= 0;
				if( id >= 3900 && id <= 3931 )//가디언 알탱이일때
				{
					AllGuardianItem.Guardian[guardian].m_Item_Guardian.bCreate		= 0;
					AllGuardianItem.Guardian[guardian].m_Item_Guardian.dwID			= 0;
				}
				else
				{
					if( AllGuardianItem.Guardian[guardian].m_Item_Guardian.dwID == 0 )
					{
						continue;
					}

					sprintf( szQuery,"Select [exp] From GuardianInfo Where [ID] = %d", AllGuardianItem.Guardian[guardian].m_Item_Guardian.dwID );
					ret = m_pDb->OpenRecord(szQuery,&dwGuardianExp,1,m_CORUM_GAME_DB[sel]);

					if( ret >= 0 )
					{
						AllGuardianItem.Guardian[guardian].m_Item_Guardian.wComplete = dwGuardianExp;
						AllGuardianItem.Guardian[guardian].m_Item_Guardian.dwEndLife = GetDueDay(525600);//pBaseItem->BaseITem_Guardian.dwDueDayTime);
					}
				}
			}			
		}
		...
		if( bChange )
		{
			//가디언 초기화가 끝났다. 여기서 업데이트...
			int			nColumn			= 2;	// 파라미터 갯수 지정 
			DWORD		dwParamOffset	= 0;	// 파라미터 Binding 정보 구조체에서 각 파라미터 Value들의 Offset
			DBBINDING*	pBinding		= m_pDb->CreateParamInfo(nColumn);		
			if(!pBinding)	continue;
			
			sprintf(szQuery,"Update CharacterInfo Set [Inv_large]=?, [Guardian]=? Where [Name]='%s' ",Name);
			
			pBinding[0].obValue		= dwParamOffset;	
			pBinding[0].cbMaxLen	= sizeof(AllGuardianItem.Inv_Large);
			pBinding[0].wType		= DBTYPE_BYTES;
			dwParamOffset			+= sizeof(AllGuardianItem.Inv_Large);		
			
			pBinding[1].obValue		= dwParamOffset;	
			pBinding[1].cbMaxLen	= sizeof(AllGuardianItem.Guardian);
			pBinding[1].wType		= DBTYPE_BYTES;
			dwParamOffset			+= sizeof(AllGuardianItem.Guardian);

			m_pDb->ExecuteSQLByParam(szQuery,pBinding,&AllGuardianItem,nColumn,m_CORUM_GAME_DB[sel]);
			m_pDb->ReleaseParamInfo(pBinding);
		}
	}
...
//	char ID[20] = {0,};
	...
	for( i = 0 ; i < pData->Query.select.dwRowCount; i++ )
	{
		...
		for( int banklarge = 0; banklarge < MAX_BANK_LARGE; banklarge++ )
		{
			...
			{
				bChange	= TRUE;
				BankInfo.Bank_Large[banklarge].m_Item_Guardian.dwEndLife = 0;
				BankInfo.Bank_Large[banklarge].m_Item_Guardian.wComplete = 0;
				if( id >= 3900 && id <= 3931 )//가디언 알탱이일때
				{
					BankInfo.Bank_Large[banklarge].m_Item_Guardian.bCreate		= 0;
					BankInfo.Bank_Large[banklarge].m_Item_Guardian.dwID			= 0;
				}
				else
				{
					if( BankInfo.Bank_Large[banklarge].m_Item_Guardian.dwID == 0 )
						continue;
					
					sprintf( szQuery,"Select [exp] From GuardianInfo Where [ID] = %d", BankInfo.Bank_Large[banklarge].m_Item_Guardian.dwID );
					ret = m_pDb->OpenRecord(szQuery,&dwGuardianExp,1,m_CORUM_GAME_DB[sel]);
					
					if( ret > 0 )
					{
						BankInfo.Bank_Large[banklarge].m_Item_Guardian.wComplete = dwGuardianExp;
						BankInfo.Bank_Large[banklarge].m_Item_Guardian.dwEndLife = GetDueDay(525600);
					}
				}
			}	
		}
		...
		if( bChange )
		{
			...
			g_pDb->ExecuteSQLByParam(szQuery,pBinding,&BankInfo,nColumn,m_CORUM_GAME_DB[sel]);
			g_pDb->ReleaseParamInfo(pBinding);
		}
	}
	...
	AfxMessageBox("Guardian Initialize End!!!",MB_OK|MB_ICONINFORMATION);
}
...
// 가디언 경험치에 따른 레벨 보상
//========================================================================================================================
void CCopyItemDelete::GuardianLevelInitialize()
{
	char				szQuery[1000]	= {0,};
	char				Name[20]		= {0,};
//	char				ID[20]			= {0,};
	...
	DWORD				nameCount		= 0;
//	DWORD				bankCount		= 0;
	DWORD				dwGuardianCount	= 0;
	DBRECEIVEDATA*		pData			= NULL;
	int					sel				= m_ComBoCtrl.GetCurSel();
	CString				ShowQuery;
//	ALLGUARDIANITEM		AllGuardianItem;
//	BankItem3			BankInfo;
	GUARDIAN_INFO_LE	GuardianInfo;
	DWORD				dwGuardianExp	= 0;
	DWORD				dwGuardianLevel	= 0;
	int					ret				= 0;
	int					i				= 0;
	GUARDIAN_BASE_LV	GuardianBaseLv[8];
	GUARDIAN_GU_INFO	GuardianGu;

	//Guardian ID
	ShowQuery.Format("All GuardianInfo Query");
	m_pEdit->SetWindowText(ShowQuery);

	sprintf(szQuery,"SELECT [ID],[baseHP],[baseSP],[LHP],[LSP] FROM BaseGuardianInfo");
	ret = m_pDb->OpenRecord(szQuery,&GuardianBaseLv, 8, m_CORUM_GAME_DB[sel] );
	
	sprintf(szQuery,"SELECT Count(*) FROM GuardianInfo");	
	ret = m_pDb->OpenRecord(szQuery,&dwGuardianCount,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[sel]);
	sprintf(szQuery,"SELECT [ID] FROM GuardianInfo");
	pData = m_pDb->OpenRecordEx(szQuery,dwGuardianCount,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[sel]);
	...
	//수호가디언은 빼자
	for( i = 1; i < pData->Query.select.dwRowCount; i++ )
	{
		if(!m_bRun)
		{
			m_pDb->ReleaseRecordset(pData);
...
//		memset(	ID,0,sizeof(ID)	);
		memcpy(	&ID,pData->Query.select.pResult + ( i * pData->Query.select.dwRowSize ),pData->Query.select.dwRowSize);
//		memcpy(	&ID,pData->Query.select.pResult + ( 1 * pData->Query.select.dwRowSize ),pData->Query.select.dwRowSize);
//		sprintf( ID, "hon21c");
//		ID	= 842;
		
		ShowQuery.Format("Q(GuardianInfo):<%d>[%d].[%d]",pData->Query.select.dwRowCount,i+1,ID);
		m_pEdit->SetWindowText(ShowQuery);

		dwGuardianExp	= 0;
		dwGuardianLevel	= 0;
		sprintf(szQuery,"Select [Exp],[BaseIndex] From GuardianInfo Where [ID]= %d",ID);
		int ret = m_pDb->OpenRecord(szQuery,&GuardianGu,1,m_CORUM_GAME_DB[sel]);
		...
		if( GuardianGu.dwExp <= 10000000 )
		{
			GuardianInfo.dwExp		= 995000;
			GuardianInfo.dwLevel	= 50;
		}
		else if( GuardianGu.dwExp <= 50000000 )
		{
			GuardianInfo.dwExp		= 7395000;
			GuardianInfo.dwLevel	= 80;
		}
		else if( GuardianGu.dwExp <= 100000000 )
		{
			GuardianInfo.dwExp		= 19120000;
			GuardianInfo.dwLevel	= 100;
		}
		else if( GuardianGu.dwExp <= 200000000 )
		{
			GuardianInfo.dwExp		= 52400000;
			GuardianInfo.dwLevel	= 120;
		}
		else// if( dwGuardianExp > 200000000 )
		{
			GuardianInfo.dwExp		= 79690000;
			GuardianInfo.dwLevel	= 130;
		}

		int nNumber = 0;

		switch( GuardianGu.dwBaseIndex )
		{
		case 5020:
			{
				nNumber = 0;
		...
		GuardianInfo.dwHP = GuardianBaseLv[nNumber].wBaseHp + ( GuardianBaseLv[nNumber].byHp * GuardianInfo.dwLevel );
		GuardianInfo.dwMp = GuardianBaseLv[nNumber].wBaseSp + ( GuardianBaseLv[nNumber].byMp * GuardianInfo.dwLevel );
		
		int			nColumn			= 4;	//파라미터 갯수 지정 
		DWORD		dwParamOffset	= 0;	//파라미터 Binding 정보 구조체에서 각 파라미터 Value들의 Offset
		DBBINDING*	pBinding		= g_pDb->CreateParamInfo(nColumn);
		if(!pBinding)	continue;
		
		sprintf(szQuery,"Update GuardianInfo Set Level = ?, Exp = ?, Hp = ?, Mp = ? Where [ID]= %d",ID);
		
		pBinding[0].obValue		= dwParamOffset;
		pBinding[0].cbMaxLen	= sizeof(GuardianInfo.dwLevel);
		pBinding[0].wType		= DBTYPE_I4;
		dwParamOffset			+= sizeof(GuardianInfo.dwLevel);

		pBinding[1].obValue		= dwParamOffset;
		pBinding[1].cbMaxLen	= sizeof(GuardianInfo.dwExp);
		pBinding[1].wType		= DBTYPE_I4;
		dwParamOffset			+= sizeof(GuardianInfo.dwExp);

		pBinding[2].obValue		= dwParamOffset;
		pBinding[2].cbMaxLen	= sizeof(GuardianInfo.dwHP);
		pBinding[2].wType		= DBTYPE_I4;
		dwParamOffset			+= sizeof(GuardianInfo.dwHP);

		pBinding[3].obValue		= dwParamOffset;
		pBinding[3].cbMaxLen	= sizeof(GuardianInfo.dwMp);
		pBinding[3].wType		= DBTYPE_I4;
		dwParamOffset			+= sizeof(GuardianInfo.dwMp);
		
		g_pDb->ExecuteSQLByParam(szQuery,pBinding,&GuardianInfo,nColumn,m_CORUM_GAME_DB[sel]);
		g_pDb->ReleaseParamInfo(pBinding);
	}
	...
	AfxMessageBox("Guardian Init End!!!",MB_OK|MB_ICONINFORMATION);
}

void CCopyItemDelete::GuardianSkillInitialize()
{
	char				szQuery[1000]	= {0,};
	char				Name[20]		= {0,};
	char				ID[20]			= {0,};
...
//	BankItem3			BankInfo;
	...
	GUARDIANSKILL		stGuardianSkill[8];
//	GUARDIANSKILL		stGuardianSkill[1];
	DWORD				dwGuardianCount	= 0;
	GUARDIAN_SKILL_INFO	stGuardianInfo;

	memset( stGuardianSkill, 0, sizeof(stGuardianSkill) );

	//캐릭터 이름
	ShowQuery.Format("All GuardianInfo Query");
	m_pEdit->SetWindowText(ShowQuery);

	sprintf(szQuery,"SELECT [skill1], [skill1_lev], [skill2], [skill2_lev] FROM BaseMonsterInfo where "
					"serial_no = 5090 or "
					"serial_no = 5100 or "
					"serial_no = 5110 or "
					"serial_no = 5120 or "
	...
	ret = m_pDb->OpenRecord( szQuery, stGuardianSkill, DEFAULT_ROWS_PER_READ, m_CORUM_GAME_DB[sel] );

	sprintf(szQuery,"SELECT Count(*) FROM GuardianInfo where OldGuardian = 1");
	ret = m_pDb->OpenRecord(szQuery,&dwGuardianCount,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[sel]);
	if( ret <= 0 )		return;

	sprintf(szQuery,"SELECT [ID], [BaseIndex], [SkillKind1], [SkillLevel1], [SkillKind2], [SkillLevel2] FROM GuardianInfo where OldGuardian = 1");
	pData = m_pDb->OpenRecordEx(szQuery,dwGuardianCount,DEFAULT_ROWS_PER_READ,m_CORUM_GAME_DB[sel]);
	...
	for( DWORD i = 0; i < pData->Query.select.dwRowCount; i++ )
	{
		...
		memset( &stGuardianInfo, 0, sizeof(stGuardianInfo) );
		memcpy( &stGuardianInfo, pData->Query.select.pResult + ( i * pData->Query.select.dwRowSize ),pData->Query.select.dwRowSize );
		
		ShowQuery.Format("Q(GuardianInfo):<%u>[%u].[%u]",pData->Query.select.dwRowCount, i+1, stGuardianInfo.dwGuardianID );
		m_pEdit->SetWindowText(ShowQuery);

		switch( stGuardianInfo.dwBaseIndex )
		{
		case 5010:
			InsertGuardianSkill( &stGuardianInfo, &stGuardianSkill[0] );		break;
		case 5020:
			InsertGuardianSkill( &stGuardianInfo, &stGuardianSkill[1] );		break;
		case 5030:
			InsertGuardianSkill( &stGuardianInfo, &stGuardianSkill[2] );		break;
		case 5040:
			InsertGuardianSkill( &stGuardianInfo, &stGuardianSkill[3] );		break;
		case 5050:
			InsertGuardianSkill( &stGuardianInfo, &stGuardianSkill[4] );		break;
		case 5060:
			InsertGuardianSkill( &stGuardianInfo, &stGuardianSkill[5] );		break;
		case 5070:
			InsertGuardianSkill( &stGuardianInfo, &stGuardianSkill[6] );		break;
		case 5080:
			InsertGuardianSkill( &stGuardianInfo, &stGuardianSkill[7] );		break;
		}
		...
		sprintf(szQuery,"Update GuardianInfo Set [SkillKind1]=?, [SkillLevel1]=?, [SkillKind2]=?, [SkillLevel2]=? Where [ID]= %u ", stGuardianInfo.dwGuardianID );
		
		pBinding[0].obValue		= dwParamOffset;	
		pBinding[0].cbMaxLen	= sizeof(stGuardianInfo.bySkillKind1);
		pBinding[0].wType		= DBTYPE_UI1;
		dwParamOffset			+= sizeof(stGuardianInfo.bySkillKind1);		
		
		pBinding[1].obValue		= dwParamOffset;	
		pBinding[1].cbMaxLen	= sizeof(stGuardianInfo.bySkillLevel1);
		pBinding[1].wType		= DBTYPE_UI1;
		dwParamOffset			+= sizeof(stGuardianInfo.bySkillLevel1);

		pBinding[2].obValue		= dwParamOffset;	
		pBinding[2].cbMaxLen	= sizeof(stGuardianInfo.bySkillKind2);
		pBinding[2].wType		= DBTYPE_UI1;
		dwParamOffset			+= sizeof(stGuardianInfo.bySkillKind2);		
		
		pBinding[3].obValue		= dwParamOffset;	
		pBinding[3].cbMaxLen	= sizeof(stGuardianInfo.bySkillLevel2);
		pBinding[3].wType		= DBTYPE_UI1;
		dwParamOffset			+= sizeof(stGuardianInfo.bySkillLevel2);

		m_pDb->ExecuteSQLByParam( szQuery, pBinding, &stGuardianInfo.bySkillKind1, nColumn, m_CORUM_GAME_DB[sel] );
		m_pDb->ReleaseParamInfo(pBinding);
	}
	...
	AfxMessageBox("Guardian Skill Insert End!!!",MB_OK|MB_ICONINFORMATION);
}

void CCopyItemDelete::InsertGuardianSkill( GUARDIAN_SKILL_INFO* pGSkillInfo, GUARDIANSKILL* pBaseSkill )
{
	pGSkillInfo->bySkillKind1	= pBaseSkill->bySkill1;
	pGSkillInfo->bySkillKind2	= pBaseSkill->bySkill2;
	pGSkillInfo->bySkillLevel1	= pBaseSkill->bySkillLevel1;
	pGSkillInfo->bySkillLevel2	= pBaseSkill->bySkillLevel2;
}

void IncorrectItemLog( CItem2* pItem, CItem_Ex* pNewItem, const string &rstrName )
{
	memcpy( &pNewItem->m_wItemID,&pItem->m_wItemID		, sizeof( BYTE ) * 10 );
	memcpy( &pNewItem->m_bSlot	,&pItem->m_bSlot		, sizeof( BYTE ) * 17 );

	char szBuf[MAX_PATH]= {0,};

	switch( pItem->GetKind() )
	{
	case __ITEM_WEAPON__	:
		{
			pNewItem->m_Item_Weapon.bReserved		= 0;
			pNewItem->m_Item_Weapon.dwReserved		= 0;
			pNewItem->m_Item_Weapon.i64Reserved		= 0;
			pNewItem->m_Item_Weapon.wMin_Damage		= pItem->m_ItemWeapon.wMin_Damage;
			pNewItem->m_Item_Weapon.wMana_Damage	= pItem->m_ItemWeapon.wMana_Damage;
			pNewItem->m_Item_Weapon.wMax_Damage		= pItem->m_ItemWeapon.wMax_Damage;

			if( pNewItem->m_Item_Weapon.wMin_Damage	!= pItem->m_ItemWeapon.wMin_Damage
			||( pNewItem->m_Item_Weapon.wMana_Damage != pItem->m_ItemWeapon.wMana_Damage)
			||( pNewItem->m_Item_Weapon.wMax_Damage	!= pItem->m_ItemWeapon.wMax_Damage))
			{
				g_Log.Log( LOG_LV0, "%s	ID=	%d	Serial= %s	Min=	%d	Max=	%d	S_DMG=	%d",
					rstrName.c_str(), 
					pItem->m_wItemID, 
					_i64toa(pItem->m_Serial.i64Serial, szBuf, 10),
					pItem->m_ItemWeapon.wMin_Damage, 
					pItem->m_ItemWeapon.wMax_Damage,
					pItem->m_ItemWeapon.wMana_Damage ); 
			}
		}break;
	case __ITEM_AROMR__	:
		{
			pNewItem->m_Item_Armor.wDefense			= pItem->m_Item_Armor.wDefense;
			pNewItem->m_Item_Armor.wReserved		= 0;

			if( pNewItem->m_Item_Armor.wDefense	!= pItem->m_Item_Armor.wDefense //)
			||	(pNewItem->m_wItemID == 2285) 
			||	(pNewItem->m_wItemID == 3052)  )
			{
				g_Log.Log( LOG_LV0, "%s	ID=	%d	Serial= %s	DEF=	%d",
					rstrName.c_str(), 
					pItem->m_wItemID, 
					_i64toa(pItem->m_Serial.i64Serial, szBuf, 10),
					pItem->m_Item_Armor.wDefense); 
			}
		}break;
	case __ITEM_SUPPLIES__	:
		{
			pNewItem->m_Item_Supplies.bQuantity		= pItem->m_Item_Supplies.bQuantity;
			pNewItem->m_Item_Supplies.bReserved		= 0;
		}break;
	case __ITEM_ZODIAC__	:
		{
			pNewItem->m_Item_Zodiac.wPosX			= pItem->m_Item_Zodiac.wPosX;
			pNewItem->m_Item_Zodiac.wPosZ			= pItem->m_Item_Zodiac.wPosZ;
			pNewItem->m_Item_Zodiac.bQuantity		= pItem->m_Item_Zodiac.bQuantity;
			pNewItem->m_Item_Zodiac.bType			= pItem->m_Item_Zodiac.bType;
			pNewItem->m_Item_Zodiac.wMapId			= pItem->m_Item_Zodiac.wMapId;
			pNewItem->m_Item_Zodiac.bLayer			= pItem->m_Item_Zodiac.bLayer;
			pNewItem->m_Item_Zodiac.bInvIndex		= 0;
			pNewItem->m_Item_Zodiac.bReserved		= 0;
		}break;
	case __ITEM_RIDE__	:	// 바뀐게 없다 - -;;
		{
			pNewItem->m_Item_RideObject.fWorldPos_x	= pItem->m_Item_RideObject.fWorldPos_x;
			pNewItem->m_Item_RideObject.fWorldPos_z	= pItem->m_Item_RideObject.fWorldPos_z;
		}break;
	case __ITEM_SPECIAL__	:
		{
			pNewItem->m_Item_Special.bQuantity		= pItem->m_Item_Special.bQuantity;
			pNewItem->m_Item_Special.bReserved		= 0;
		}break;
	case __ITEM_GUARDIAN__	:
		{
			pNewItem->m_Item_Guardian.dwEndLife			= pItem->m_Item_Guardian.dwEndLife;
			pNewItem->m_Item_Guardian.dwCompleteTime	= pItem->m_Item_Guardian.wComplete;
			pNewItem->m_Item_Guardian.wID				= pItem->m_Item_Guardian.dwID;
			pNewItem->m_Item_Guardian.bCreate			= pItem->m_Item_Guardian.bCreate;
			pNewItem->m_Item_Guardian.bReserved			= 0;

			if(	pNewItem->m_Item_Guardian.dwEndLife			!= pItem->m_Item_Guardian.dwEndLife
			||	pNewItem->m_Item_Guardian.dwCompleteTime	!= pItem->m_Item_Guardian.wComplete
            ||	pNewItem->m_Item_Guardian.wID				!= pItem->m_Item_Guardian.dwID
			||	pNewItem->m_Item_Guardian.bCreate			!= pItem->m_Item_Guardian.bCreate)
			{
				g_Log.Log( LOG_LV0, "%s	ID=	%d	Serial= %s	EndLife=	%d	CompleteTime=	%d	Create=	%d	GuardianInfoID=	%d",
						rstrName.c_str(), 
						pItem->m_wItemID, 
						_i64toa(pItem->m_Serial.i64Serial, szBuf, 10),
						pItem->m_Item_Guardian.dwEndLife, 
						pItem->m_Item_Guardian.wComplete, 
						pItem->m_Item_Guardian.bCreate, 
						pItem->m_Item_Guardian.dwID );
			}
		}break;
	case __ITEM_MAGICARRAY__:
		{
			pNewItem->m_Item_MagicArray.bQuantity		= pItem->m_Item_MagicArray.bQuantity;
			pNewItem->m_Item_MagicArray.bReserved		= 0;
		}break;
	case __ITEM_MATERIALS__	:
		{
			pNewItem->m_Item_Materials.bQuantity		= pItem->m_Item_Materials.bQuantity;
			pNewItem->m_Item_Materials.bReserved		= 0;
		}break;
	case __ITEM_MIX_UPGRADE__		:
		{
			pNewItem->m_Item_Mix_Upgrade.bQuantity		= pItem->m_Item_Mix_Upgrade.bQuantity;
			pNewItem->m_Item_Mix_Upgrade.bReserved		= 0;
		}break;
	case __ITEM_MAGICFIELD_ARRAY__	:
		{
			pNewItem->m_Item_MagicField_Array.bQuantity	= pItem->m_Item_MagicField_Array.bQuantity;
			pNewItem->m_Item_MagicField_Array.bReserved	= 0;
		}break;
	case __ITEM_CONSUMABLE__		:
		{
			pNewItem->m_Item_Consumable.dwEndDate		= pItem->m_ItemConsumable.dwEndDate;
			pNewItem->m_Item_Consumable.bInvIndex		= pItem->m_ItemConsumable.bInvIndex;
			pNewItem->m_Item_Consumable.bQuantity		= pItem->m_ItemConsumable.bQuantity;

			
			if(	pNewItem->m_Item_Consumable.dwEndDate		!= pItem->m_ItemConsumable.dwEndDate
			||	pNewItem->m_Item_Consumable.bInvIndex		!= pItem->m_ItemConsumable.bInvIndex
			||	pNewItem->m_Item_Consumable.bQuantity		!= pItem->m_ItemConsumable.bQuantity)
			{
				g_Log.Log( LOG_LV0, "%s	ID=	%d	Serial= %s	EndLife=	%d	InvIndex=	%d	Quantity=	%d",
					rstrName.c_str(), 
					pItem->m_wItemID, 
					_i64toa(pItem->m_Serial.i64Serial, szBuf, 10),
					pItem->m_ItemConsumable.dwEndDate, 
					pItem->m_ItemConsumable.bInvIndex, 
					pItem->m_ItemConsumable.bQuantity	);
			}
		}break;
	case __ITEM_UPGRADE__	:
		{
			pNewItem->m_Item_UpGreade.bQuantity			= pItem->m_Item_UpGreade.bQuantity;
			pNewItem->m_Item_UpGreade.bReserved			= 0;
		}break;	
	case __ITEM_LIQUID__	:
		{
			pNewItem->m_Item_LiQuid.bQuantity			= pItem->m_Item_LiQuid.bQuantity;
			pNewItem->m_Item_LiQuid.bReserved			= 0;
		}break;
	case __ITEM_EDITION__	:
		{
			pNewItem->m_Item_Edition.bQuantity			= pItem->m_Item_Edition.bQuantity;
			pNewItem->m_Item_Edition.bReserved			= 0;
		}break;
	case __ITEM_BAG__		:
		{
			pNewItem->m_Item_Bag.bType					= pItem->m_Item_Bag.bType;
			pNewItem->m_Item_Bag.wProbability			= pItem->m_Item_Bag.wProbability;
		}break;
	default:
		{
			g_Log.Log( LOG_LV0, "%s	ID=	%d	Serial= %s",
					rstrName.c_str(), 
					pItem->m_wItemID, 
					_i64toa(pItem->m_Serial.i64Serial, szBuf, 10)	); 
		}break;
...
WORD	GetKind(const WORD m_wItemID)
{
...
void ItemCheck( CItem_Ex* pItem, const string &rstrName )
{
	char szBuf[MAX_PATH]= {0,};

	switch( GetKind( pItem->m_wItemID ) )
	{
	case __ITEM_WEAPON__	:
		{
			ITEM_WEAPON_EX &rItem = pItem->m_Item_Weapon;
			if( ( 0		== rItem.wMin_Damage)
			||	( 0		== rItem.wMax_Damage)
			||	( 700	<= rItem.wMana_Damage)
			||	( 100	<= rItem.wMax_Damage)
			||	( 0		!= rItem.bReserved)
			||	( 0 	!= rItem.dwReserved)
			||	( 0 	!= rItem.i64Reserved) 
			||  ( rItem.wMin_Damage > rItem.wMax_Damage) )
			{
				_i64toa(pItem->m_Serial.i64Serial, szBuf, 10);
				g_Log.Log( LOG_LV0, "%s	ID=	%d	Serial= %s	Min=	%I64d	Max=	%I64d	S_DMG=	%I64d",
					rstrName.c_str(), pItem->m_wItemID, 
					szBuf,
					rItem.wMin_Damage, rItem.wMax_Damage, rItem.wMana_Damage ); 
			}
		}break;
	case __ITEM_AROMR__	:
		{
			ITEM_ARMOR_EX &rItem = pItem->m_Item_Armor;
			if( (200 < rItem.wDefense)
			||	(0 != rItem.wReserved) )
			{
				g_Log.Log( LOG_LV0, "%s	ID=	%d	Serial= %s	DEF=	%d",
					rstrName.c_str(), 
	...
	case __ITEM_GUARDIAN__	:
		{
//			if(pItem->m_Item_Guardian.bReserved			!= 0)
			{//가디언은 무조건 로그 남기게.
				g_Log.Log( LOG_LV0, "%s	ID=	%d	Serial= %s	EndLife=	%d	CompleteTime=	%d	Create=	%d	GuardianInfoID=	%d	ReserveDay=	%d",
						rstrName.c_str(), 
						pItem->m_wItemID, 
						_i64toa(pItem->m_Serial.i64Serial, szBuf, 10),
						pItem->m_Item_Guardian.dwEndLife, 
						...
						pItem->m_Item_Guardian.bCreate, 
						pItem->m_Item_Guardian.wID,
						GetReserveDay(pItem->m_Item_Guardian.dwEndLife) );
			}
		}break;
	}
}

void LogWork(const int MAX_INDEX, vector<BYTE> &src, const string &rstrName)
{
	for( int index = 0 ; index < MAX_INDEX; index++ )
	{
		CItem_Ex *pItem =  reinterpret_cast<CItem_Ex*>(&src.at( index * sizeof(CItem_Ex) )) ;
		if( 0 == pItem->m_wItemID )
		{
			continue;
		}
		ItemCheck(pItem, rstrName);
	} // for( int index = 0 ; index < MAX_EQUIP ; index++ )
}

const int MAX_TRAN_COUNT = 1000;

class CTran
{
public:
	CTran(CDBConnection &rDBConn )
		: m_rDBConn(rDBConn)
	{
		BeginTran();
	}
	...
	CDBConnection &m_rDBConn;

	void BeginTran()
	{
		CArgQuery TranQuery( m_rDBConn.DataConnection() );
		TranQuery.SetQuery("begin tran");

		if( TranQuery.Execute( false ) )
		{
		}
	}
	...
	{
		CArgQuery TranQuery( m_rDBConn.DataConnection() );
		TranQuery.SetQuery("commit tran");
	...
	struct ITEM_WEAPON_EX
{
	...
	size_t size_weapon_ex = sizeof( ITEM_WEAPON_EX );
	size_t size_weapon = sizeof( ITEM_WEAPON );
*/

void CCopyItemDelete::ItemLog()		// 새로운 아이템 구조로...
{//백업된 쪽으로 데이터를 밀어 넣습니다.
	char szQuery[MAX_PATH]	= {0,};
	const int sel = m_ComBoCtrl.GetCurSel();

	const string strFile = "Data.txt";
	char szFileName[MAX_PATH] = {0,};
	sprintf(szFileName, "%d%s", sel, strFile.c_str()  );
	g_Log.Init(OUTPUT_JUST_FILE, "./Log", szFileName);
	...
	ShowQuery.Format("All Character Item");
	m_pEdit->SetWindowText(ShowQuery);
{
	DWORD dwCharCount = 0;
	{
		sprintf(szQuery,"Select Count(*) From CharacterInfo");
		CDBConnection &rDBConn = g_DBConn[sel];
		CArgQuery Query( rDBConn.DataConnection() );
		...
		{	
			...
			{
				bIsOk = Query.GetResult( 1, dwCharCount);
			}
	...
	{
		sprintf(szQuery,"Select [Char_Index],[name], [Equip],[Inv_large],[Inv_small],[Belt],[Guardian] From CharacterInfo order by [name]");
		CDBConnection &rDBConn = g_DBConn[sel];
		CArgQuery Query( rDBConn.DataConnection() );
		Query.SetQuery(szQuery);
		ULONG TotalCount = 0;
...
//				CTran Tran(rDBConn);
				...
				do
				{
					...
					bIsOk = Query.GetResult( ++index, dwCharIndex);
				
					bIsOk = Query.GetResult( ++index, strName);
					
					bIsOk = Query.GetResult( ++index, vecEquip);
					bIsOk = Query.GetResult( ++index, vecInvLarge);

					bIsOk = Query.GetResult( ++index, vecInvSmall);
					bIsOk = Query.GetResult( ++index, vecBelt);
					bIsOk = Query.GetResult( ++index, vecGuardian);
					
					ShowQuery.Format("Q[Char Convert] Acc:[%d], Work Index:[%d]:", TotalCount, dwCharIndex);
					m_pEdit->SetWindowText(ShowQuery);

					LogWork(MAX_EQUIP,			vecEquip,		strName);
					LogWork(MAX_INV_LARGE,		vecInvLarge,	strName);
					LogWork(MAX_INV_SMALL,		vecInvSmall,	strName);
					LogWork(MAX_BELT,			vecBelt,		strName);
					LogWork(MAX_INV_GUARDIAN,	vecGuardian,	strName);
		...
		{
			assert(NULL && "Char Info Count fault");
		}
	}
}
	g_Log.Log( LOG_LV1, "======================== Bank Info Log] ========================" );

{
	ShowQuery.Format("Q[Bank Info Start]");
	m_pEdit->SetWindowText(ShowQuery);
	...
	sprintf(szQuery,"Select Count(*) From BankInfo");
	{
		...
		{	
			...
			if(S_OK == Query.MoveFirst())
			{
				bIsOk = Query.GetResult( 1, dwBankinfoCount);
			}
		} // if( Query.Execute( szQuery ) )
	}

	{
		sprintf(szQuery,"Select [ID],[Bank_large],[Bank_small] From BankInfo order by [ID]");
...
//			CTran Tran(rDBConn);
			...
			if(S_OK == Query.MoveFirst())
			{
				do
				{
					...
					bIsOk = Query.GetResult( ++index, strID);
					bIsOk = Query.GetResult( ++index, vecBankLarge);
					bIsOk = Query.GetResult( ++index, vecBankSmall);
					
					ShowQuery.Format("Q[Bank Convert] Acc:[%d], Work Index:[%s]:", TotalCount, strID.c_str() );
					m_pEdit->SetWindowText(ShowQuery);

					LogWork(MAX_BANK_LARGE,		vecBankLarge,	strID);
					LogWork(MAX_BANK_SMALL,		vecBankSmall,	strID);
	...
	ShowQuery.Format("All Work Complete" );
	m_pEdit->SetWindowText(ShowQuery);

	// 다 끝났다.
	AfxMessageBox("NewItemConvert End!!!",MB_OK|MB_ICONINFORMATION);
}
...
void ConvertWork(const int MAX_INDEX, vector<BYTE> &src, vector<BYTE> &tgt)
{
	for( int index = 0 ; index < MAX_INDEX; index++ )
	{
		CItem2 *pItem =  reinterpret_cast<CItem2*>(&src.at( index * sizeof(CItem2) )) ;
		if( 0 == pItem->GetID()  )
		{
			continue;
		}

		CItem_Ex *pItem_ex = reinterpret_cast<CItem_Ex*>(&tgt.at( index * sizeof(CItem2) )) ;
		::ItemConvert( pItem, pItem_ex );
	} // for( int index = 0 ; index < MAX_EQUIP ; index++ )
}

void CCopyItemDelete::ConvertNewItem(const char* szBackupAddName)		// 새로운 아이템 구조로...
{//백업된 쪽으로 데이터를 밀어 넣습니다.
	char szQuery[MAX_PATH]	= {0,};
	const int sel = m_ComBoCtrl.GetCurSel();
...

{
	...
	{
		sprintf(szQuery,"Select [Char_Index],[Equip],[Inv_large],[Inv_small],[Belt],[Guardian] From CharacterInfo%s", szBackupAddName);
		CDBConnection &rDBConn = g_DBConn[sel];
		CArgQuery Query( rDBConn.DataConnection() );
		Query.SetQuery(szQuery);
		ULONG TotalCount = 0;
...
//				CTran Tran(rDBConn);
				...
				do
				{
					...
					vector<BYTE> vecEquip_ex(SIZE_OF_EQUIP ,0);
					vector<BYTE> vecInvLarge_ex(SIZE_OF_LARGE,0);
					vector<BYTE> vecInvSmall_ex(SIZE_OF_SMALL,0);
					vector<BYTE> vecBelt_ex(SIZE_OF_BELT,0);
					vector<BYTE> vecGuardian_ex(SIZE_OF_GUARDIAN,0);

					ConvertWork(MAX_EQUIP,			vecEquip,		vecEquip_ex);
					ConvertWork(MAX_INV_LARGE,		vecInvLarge,	vecInvLarge_ex);
					ConvertWork(MAX_INV_SMALL,		vecInvSmall,	vecInvSmall_ex);
					ConvertWork(MAX_BELT,			vecBelt,		vecBelt_ex);
					ConvertWork(MAX_INV_GUARDIAN,	vecGuardian,	vecGuardian_ex);
					{
//						if(0 ==( TotalCount % MAX_TRAN_COUNT) )
						{
//							ShowQuery.Format("Q[Char Convert] Acc:[%d], Work Index:[%d]:", TotalCount, dwCharIndex);
//							m_pEdit->SetWindowText(ShowQuery);
...
//							Tran.BeginTran();
						...
						sprintf(szQuery, "Update CharacterInfo Set Equip=?, Inv_large=?, Inv_small=?, Belt=?, Guardian=? Where Char_Index = %d", dwCharIndex);
						
						CArgQuery UpdateQuery( rDBConn.DataConnection() );
						if( UpdateQuery.SetQuery( szQuery ) )
						{
						//	UpdateQuery.SetBinary( 1, vecEquip_ex );
						//	UpdateQuery.SetBinary( 2, vecInvLarge_ex );
						//	UpdateQuery.SetBinary( 3, vecInvSmall_ex );
						//	UpdateQuery.SetBinary( 4, vecBelt_ex );
						//	UpdateQuery.SetBinary( 5, vecGuardian_ex );
							...
							{
								assert(NULL && "Char Info UpdateFail");
							}
						} // if( UpdateQuery.SetQuery( szQuery ) )
					}
				}while ( S_OK == Query.MoveNext() );
...

{
	...
	{
		sprintf(szQuery,"Select [ID],[Bank_large],[Bank_small] From BankInfo%s", szBackupAddName);
...
//			CTran Tran(rDBConn);
			...
			if(S_OK == Query.MoveFirst())
			{
				do
				{
					...
					vector<BYTE> vecBankLarge_ex(SIZE_OF_BANK_LARGE, 0);
					vector<BYTE> vecBankSmall_ex(SIZE_OF_BANK_SMALL, 0);

					ConvertWork(MAX_BANK_LARGE,		vecBankLarge,	vecBankLarge_ex);
					ConvertWork(MAX_BANK_SMALL,		vecBankSmall,	vecBankSmall_ex);
...
//							Tran.BeginTran();
						...
						sprintf(szQuery, "Update BankInfo Set Bank_large=?, Bank_small=? Where ID = '%s'", strID.c_str());
						
						CArgQuery UpdateQuery( rDBConn.DataConnection() );
						UpdateQuery.SetQuery( szQuery );

						UpdateQuery.SetBinary( 1, vecBankLarge_ex );
						UpdateQuery.SetBinary( 2, vecBankSmall_ex );

						if(! UpdateQuery.Execute(false) )
						{
							assert(NULL && "Bank Info UpdateFail");
						}
					}
				}while ( S_OK == Query.MoveNext() );
			} // if(S_OK == Query.MoveFirst())
