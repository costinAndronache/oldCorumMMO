...
CRITICAL_SECTION g_ExceptionCS;
...
{
	EnterCriticalSection(&g_ExceptionCS);

	// Initialize symbols..
	SymSetOptions(SYMOPT_DEFERRED_LOADS | SYMOPT_UNDNAME | SYMOPT_LOAD_LINES);
	if(SymInitialize(GetCurrentProcess(), NULL, TRUE) == FALSE) 
	{
		LeaveCriticalSection(&g_ExceptionCS);
	...
	return EXCEPTION_CONTINUE_SEARCH; // 일단 여기서 로그는 남기고, 외부에서 다시.. 잡히도록 한다.
}
